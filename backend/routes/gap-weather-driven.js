...
// Route: /api/gap-weather-driven
// Feature: Weather Driven
// Description: Weather-driven heat-stress alerts (no weather route)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Weather Driven. Weather-driven heat-stress alerts (no weather route)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'weather-driven', title: 'Weather Driven', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'weather-driven', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['weather-driven']);
      return res.json({ slug: 'weather-driven', items: r.rows || [] });
    }
    return res.json({ slug: 'weather-driven', items: [] });
  } catch (e) {
    return res.json({ slug: 'weather-driven', items: [], error: (e && e.message) || 'Server error' });
  }
});
