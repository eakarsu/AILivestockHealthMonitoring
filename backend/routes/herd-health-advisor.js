// Auto-generated feature route: herd-health-advisor
// Domain: veterinary herd advisor — Capability: recommending vaccinations, dosing, and seasonal risk mitigations
// Mounted under: /api/herd-health-advisor
...
// POST /api/herd-health-advisor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a veterinary herd advisor. Your task is recommending vaccinations, dosing, and seasonal risk mitigations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/herd-health-advisor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/herd-health-advisor/:id — fetch one
...
// DELETE /api/herd-health-advisor/:id — discard a run
