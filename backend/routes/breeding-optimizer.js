// Auto-generated feature route: breeding-optimizer
// Domain: animal genetics advisor — Capability: optimizing breeding pairs with inbreeding coefficient tracking
// Mounted under: /api/breeding-optimizer
...
// POST /api/breeding-optimizer/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a animal genetics advisor. Your task is optimizing breeding pairs with inbreeding coefficient tracking. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/breeding-optimizer/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/breeding-optimizer/:id — fetch one
...
// DELETE /api/breeding-optimizer/:id — discard a run
