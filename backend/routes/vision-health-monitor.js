// Auto-generated feature route: vision-health-monitor
// Domain: livestock vision analyst — Capability: detecting lameness, body condition, and distress from video frames
// Mounted under: /api/vision-health-monitor
  ...
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Title': 'Livestock Health Monitoring'
      }
    };
...
// POST /api/vision-health-monitor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a livestock vision analyst. Your task is detecting lameness, body condition, and distress from video frames. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vision-health-monitor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vision-health-monitor/:id — fetch one
...
// DELETE /api/vision-health-monitor/:id — discard a run
