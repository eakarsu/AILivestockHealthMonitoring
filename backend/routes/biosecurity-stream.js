// Auto-generated feature route: biosecurity-stream
// Domain: biosecurity monitoring agent — Capability: streaming herd risk and quarantine recommendations
// Mounted under: /api/biosecurity-stream
...
// POST /api/biosecurity-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a biosecurity monitoring agent. Your task is streaming herd risk and quarantine recommendations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/biosecurity-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/biosecurity-stream/:id — fetch one
...
// DELETE /api/biosecurity-stream/:id — discard a run
