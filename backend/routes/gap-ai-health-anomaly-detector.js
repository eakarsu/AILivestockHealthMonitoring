...
// Route: /api/gap-ai-health-anomaly-detector
// Feature: Ai Health Anomaly Detector
// Description: (behavior/vitals anomalies)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Health Anomaly Detector. (behavior/vitals anomalies)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Health Anomaly Detector\nDescription: (behavior/vitals anomalies)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-health-anomaly-detector', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-health-anomaly-detector', title: 'Ai Health Anomaly Detector', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-health-anomaly-detector', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-health-anomaly-detector']);
      return res.json({ slug: 'ai-health-anomaly-detector', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-health-anomaly-detector', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-health-anomaly-detector', items: [], error: (e && e.message) || 'Server error' });
  }
});
