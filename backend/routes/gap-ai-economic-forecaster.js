...
// Route: /api/gap-ai-economic-forecaster
// Feature: Ai Economic Forecaster
// Description: (herd profitability projection)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Economic Forecaster. (herd profitability projection)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Economic Forecaster\nDescription: (herd profitability projection)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-economic-forecaster', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-economic-forecaster', title: 'Ai Economic Forecaster', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-economic-forecaster', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-economic-forecaster']);
      return res.json({ slug: 'ai-economic-forecaster', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-economic-forecaster', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-economic-forecaster', items: [], error: (e && e.message) || 'Server error' });
  }
});
