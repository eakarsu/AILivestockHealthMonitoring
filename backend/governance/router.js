      ...
      const result = await db.query(
        `WITH changed AS (
          UPDATE governed_integration_outbox SET
            ...
            next_attempt_at=NOW()+(INTERVAL '1 minute'*LEAST(60,POWER(2,attempts))),updated_at=NOW()
          WHERE id=$3 AND tenant_id=$4 AND status IN('queued','processing','failed') RETURNING *
        ), logged AS (
          INSERT INTO governed_work_events(tenant_id,work_item_id,actor_id,event_type,details)
          SELECT tenant_id,work_item_id,$5,'integration_result',jsonb_build_object('provider',provider,'status',status) FROM changed
        ) SELECT * FROM changed`,
        [req.body.status, String(req.body.error || '').slice(0, 1000), req.params.outboxId, ctx.tenant, ctx.actor]
      );
